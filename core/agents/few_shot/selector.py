...
selector_lib_few_shot = [
    Demo(
        ...
        selection_steps = """
"1) Operating_Conditions" is kept because:
We have to always mark the operating conditions as keep because the database mixes the information 
for all corners in the same table. 

2) "Cells" is kept because:
It contains the primary information about all gates.

3) "Input_Pins" and "Output_Pins" are dropped because:
...
"""
    ...
    Demo(
        ...
        selection_steps = """
1) Operating_Conditions" is kept because:
We must filter entires from the cells table that are related to a specific corner. 
We have to always mark the operating conditions as keep because the database mixes the information 
for all corners in the same table. 
...
```
    ...
    Demo(
        ...
        desc_str = get_desc("Liberty"),
        fk_str = get_fk("Liberty"),
        selection_steps ="""1) "Operating_Conditions" is kept because:
We have to always mark the operating conditions as keep iiregardless of the question because the database mixes the information 
for all corners in the same table. 
...
5) "Timing_Values" is kept because:
...
selector_few_shot = selector_lib_few_shot + selector_lef_few_shot + selector_tlef_few_shot
