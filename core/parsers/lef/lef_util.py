...
class Pin(Statement):
    ...
    def __init__(self, name):
        ...
        self.name = name
        self.info = {}
        self.info["ANTENNAGATEAREA"] = None 
        self.info["ANTENNADIFFAREA"] = None 
    ...
    def parse_next(self, data):
        if data[0] == "DIRECTION":
            ...
            if data[1] == self.name:
                ...
                return -1
        elif data[0] == 'ANTENNAGATEAREA':
            self.info["ANTENNAGATEAREA"] = float(data[1])
        elif data[0] == 'ANTENNADIFFAREA':
            self.info["ANTENNADIFFAREA"] = float(data[1])
...
class Layer(Statement):
    ...
    def __init__(self, name):
        ...
        self.edge_cap = None
        self.property = None
        self.dc_current_density = None 
        self.ac_current_density = None
        self.max_density = None
        self.antenna_model = None
        self.antenna_gate_area = None
        self.antenna_diff_area = None
        self.min_enclosed_area = None
        self.area = None 
        self.density_check_window = None 
        self.density_check_step = None 
        self.enclosures = [] 
        self.antenna_diff_side_area_ratio = None 
        self.antenna_diff_area_ratio = None 
    ...
    def parse_next(self, data):
        ...
        elif data[0] == 'DCCURRENTDENSITY':
            self.dc_current_density = (data[1], float(data[2]))
        elif data[0] == 'ACCURRENTDENSITY':
            self.ac_current_density = (data[1], float(data[2]))
        elif data[0] == 'MAXIMUMDENSITY':
            self.max_density = float(data[1])
        elif data[0] == 'ANTENNAMODEL':
            self.antenna_model = data[1]
        elif data[0] == 'ANTENNADIFFSIDEAREARATIO':
            self.antenna_diff_side_area_ratio = [data[1], (float(data[4]), float(data[5])), (float(data[8]), float(data[9])), 
                                                 (float(data[12]), float(data[13])), (float(data[16]), float(data[17]))]
        elif data[0] == 'ANTENNADIFFAREARATIO':
            self.antenna_diff_area_ratio = [data[1], (float(data[4]), float(data[5])), (float(data[8]), float(data[9])), 
                                                        (float(data[12]), float(data[13])), (float(data[16]), float(data[17]))]
                    
        elif data[0] == 'MINENCLOSEDAREA':
            self.min_enclosed_area = data[1]
        elif data[0] == 'AREA':
            self.area = float(data[1])
        elif data[0] == 'DENSITYCHECKWINDOW':
            self.density_check_window = (float(data[1]), float(data[2])) 
        elif data[0] == 'DENSITYCHECKSTEP':
            self.density_check_step = float(data[1]) 
        elif data[0] == 'ENCLOSURE':
            try: 
                self.enclosures.append((data[1], float(data[2]), float(data[3])))
            except ValueError:
                pass 
        elif data[0] == "END":
            if data[1] == self.name:
