...
foreach inst $instances {
    ...
    if {[string first "DF" $cell_name] >= 0 || [string first "FF" $cell_name] >= 0 || [string first "LAT" $cell_name] >= 0} {
        incr sequential_cells
    }
}

# Calculate the percentage of sequential cells
if {$total_cells > 0} {
    set sequential_percentage [expr {($sequential_cells / $total_cells) * 100.0}]
} else {
    set sequential_percentage 0
}

# Report the results
puts "Total Cells in the Design: $total_cells"
puts "Sequential Cells in the Design: $sequential_cells"
puts "Percentage of Sequential Cells: $sequential_percentage"
